...
class Board:
    ...
    def get_tile_at(self, index: int) -> Tile:
        ...
        ), f"The given tile index {index} falls outside the board ranging from 0 to {len(self.tiles) - 1}"
    ...
    def get_tiles_of(self, player_id: int) -> list[BuyableTile]:
        return [
            ...
            if isinstance(tile, BuyableTile) and tile.owner == player_id
        ]
    ...
    def _load_tiles(self):
        with open("board.json", "r") as file:
            ...
            for tile in sorted_board_data:
                assert (
                    tile["type"] is not None
                ), "A tile in the board data file does not have the type key, which is required for each tile."
                assert (
                    len(self.tiles) == tile["position"]
                ), f"The tile {tile["name"]} has position {tile["position"]} while it is expected to be at {len(self.tiles)}."
                ...
                if tile["type"] == "street":
                    ...
                    self.tiles.append(
                        Railroad(
                            ...
                            land_value=tile["land_value"],
                            mortgage_value=tile["mortgage_value"],
                            rent_one_station=tile["rent_one_station"],
                            rent_two_stations=tile["rent_two_stations"],
                            rent_three_stations=tile["rent_three_stations"],
                            rent_four_stations=tile["rent_four_stations"],
                        )
                    )
                elif tile["type"] == "card":
                    self.tiles.append(
                        CardTile(name=tile["name"], card_type=tile["card_type"])
                    )
                elif tile["type"] == "go":
                    ...
                    raise ValueError(
                        f"Unknown tile type: {tile['type']} for tile {tile['name']} at position {tile['position']}"
                    )
